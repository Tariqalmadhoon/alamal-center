...
@section('title', 'تعديل استفادة - ' . $orphan->full_name)

@section('content')
<div class="page-header">
    <div>
        <h1 class="page-title">تعديل استفادة</h1>
...
<div class="content-card">
    <form action="{{ route('orphans.benefits.update', [$orphan, $benefit]) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <select name="benefit_type" class="form-control @error('benefit_type') error @enderror" required>
                    ...
                    @foreach($types as $key => $label)
                        <option value="{{ $key }}" {{ old('benefit_type', $benefit->benefit_type) == $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @error('benefit_type')
            ...
            <div class="form-group">
                ...
                <input type="date" name="benefit_date" value="{{ old('benefit_date', $benefit->benefit_date->format('Y-m-d')) }}" 
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <input type="number" name="amount" value="{{ old('amount', $benefit->amount) }}" 
            ...
            <div class="form-group">
                ...
                <input type="text" name="description" value="{{ old('description', $benefit->description) }}" 
        ...
        <div class="form-group">
            ...
            <textarea name="notes" class="form-control" rows="3" 
                      placeholder="أي ملاحظات إضافية...">{{ old('notes', $benefit->notes) }}</textarea>
        </div>
